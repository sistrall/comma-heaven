            ...
            unscoped do
              where(["#{columns.table_alias}.#{primary_key} IN (?)", ids]).limit(options[:limit]).joins(columns.joins).select(columns.select).all.each do |resource|
                fields = columns.sql_as.inject([]) do |acc, field|
                  ...
                  if options[:format]
                    begin 
                      value = value.strftime(options[:format][:date]) if value.is_a?(Date) && options[:format][:date]
                      value = value.strftime(options[:format][:datetime]) if value.is_a?(Time) && options[:format][:datetime]
                    rescue 
                    end
                  end
