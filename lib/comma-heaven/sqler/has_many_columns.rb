module CommaHeaven
  module Sqler
    class HasManyColumns < AssociationColumns
      def join_clause
        # @:through@ relationships requires special treatment: 
        ...
        join = if association.options[:through]
          <<-EOS
            ...
            LEFT JOIN #{quote(table)} AS #{table_alias}
              ON _#{association.through_reflection.table_name}#{table_alias}.#{model.primary_key} = #{table_alias}.#{foreign_key_for(association.source_reflection)}
          EOS
        else
          <<-EOS
            LEFT JOIN #{quote(table)} AS #{table_alias}
            ...
            <<-EOS
              AND #{table_alias}.#{association.klass.primary_key} = 
                ( SELECT #{association.quoted_table_name}.#{association.klass.primary_key} 
                  ...
                  JOIN #{association.through_reflection.table_name} ON #{association.quoted_table_name}.#{foreign_key_for(association.source_reflection)} = #{association.through_reflection.table_name}.#{association.through_reflection.klass.primary_key}
